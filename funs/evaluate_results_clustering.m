...
%
% Syntax:
% 		results = evaluate_results_clustering(gt,y_pred)
%
% Input:
...
% function [acc_O, NMI_sqrt, Purity, kappa,acc_A,rstl] = evaluate_results_clustering(tlabs, Trlabs)
function [results] = evaluate_results_clustering(Trlabs,tlabs)
...
nclass1 = length(uY1);
Y0 = zeros(n,1);
if nclass1 ~= max(Trlabs)
    for i = 1:nclass1
        Y0(find(Trlabs == uY1(i))) = i;
    end;
    Trlabs = Y0;
end;
...
if nclass ~= max(tlabs)
    for i = 1:nclass
        predY0(find(tlabs == uY(i))) = i;
    end;
   tlabs = predY0;
end;


tlabs = bestMap(Trlabs,tlabs);
...
acc_A = mean( p_acc );
% accuarcy
% acc_O = length(find(Trlabs == tlabs))/length(Trlabs);
% % Normalized mutual information
[A nmii(1) avgenti(1)] = compute_nmi(Trlabs,tlabs);
NMI_sqrt=nmii(1);

predLidx = unique(tlabs); 
pred_classnum = length(predLidx);
% purity
correnum = 0;
for ci = 1:pred_classnum
    incluster = Trlabs(find(tlabs == predLidx(ci)));
...
Purity = correnum/length(tlabs);

% Kappa coefficient of agreement
kappa = (acc_O - sum( sum(CM,1)*sum(CM,2) )/sum(sum(CM)).^2)/(1-sum( sum(CM,1)*sum(CM,2) )/sum(sum(CM)).^2);

%
ARI = rand_index(Trlabs, tlabs, 'adjusted');

[F1_score, Precision, Recall] = F1_Measure(Trlabs, tlabs);
% F1_Measur=[F1_score, Precision, Recall];
% rstl.F1_Measur   = F1_Measur;
% rstl.ARI   =ARI;
% rstl.p_acc   = p_acc;
% rstl.u_acc   = u_acc;
% rstl.acc_o = acc_O;
...
% results = [acc_O, NMI_sqrt, Purity, kappa,acc_A, ARI];
results = [acc_O,kappa,NMI_sqrt,Purity,ARI,F1_score,Precision, Recall];
% results = [acc_O,kappa,NMI_sqrt,Purity,ARI,F1_score];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Fscore,PrecisionM,RecallM] = F1_Measure(actual,pred,classes)
